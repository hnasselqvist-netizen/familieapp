...
**Bredden må besluttes strukturelt, ikke korrigeres visuelt i etterkant.** To tidligere forsøk viste dette tydelig: et `left:50%`-breakout-forsøk regnet posisjonen fra feil element (foreldrens bredde, ikke viewporten), og et påfølgende forsøk med `getBoundingClientRect()`-måling kompenserte for symptomet uten å løse årsaken — begge var visuelle korreksjoner rundt en grunnleggende feil arkitektur, ikke en faktisk løsning.

Den fungerende løsningen: `App` holder en `erBredFlate`-boolean, satt av `VerktoyScreen` (via en callback-prop gjennom `MerScreen`) når en administrasjonsside er den aktive undersiden. `App` bruker denne verdien til å velge innholdsområdets `maxWidth` **før** noe rendres — ingen negative marginer, ingen `left`-forskyvning, ingen måling av et elements posisjon i etterkant. Header og bunnnavigasjon (`BottomNav`, som bruker `position:fixed` — alltid relativt til viewporten, aldri til en foreldre-boks) beholder sin egen, kontrollerte, smale bredde uavhengig av innholdsområdet. Verktøy-oversikten (menyen over administrasjonssidene) er selv smal; kun de faktiske administrasjonssidene (Generator, og etter hvert Regler, Årsbudsjett, Metadata, Kontoer) utløser bred visning.
